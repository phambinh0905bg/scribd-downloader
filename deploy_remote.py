...
# Ensure UTF-8 stdout unbuffered
...
IMAGE_TAG = "scribd-downloader:latest"
...
def create_build_tar() -> bytes:
    ...
    ignore_files = {".DS_Store", "deploy_remote.py", "test_app.py"}
...
def build_docker_image(tar_bytes: bytes):
    log(f"🔨 Đang build Docker image '{IMAGE_TAG}' trên server...")
    ...
    # Timeout set to (connect=60s, read=1800s) to allow pip download to finish safely
    res = requests.post(build_url, headers=headers, data=tar_bytes, stream=True, timeout=(60, 1800))
    
    if res.status_code != 200:
        log(f"❌ Lỗi khi gọi build API ({res.status_code}): {res.text}", "ERROR")
        sys.exit(1)
    ...
    log(f"✅ Build thành công Docker image: {IMAGE_TAG}", "SUCCESS")
...
def create_and_start_container():
    log(f"🚀 Khởi tạo container '{CONTAINER_NAME}' (Port {HOST_PORT}:8000)...")
    ...
    log(f"✅ Container '{CONTAINER_NAME}' đã khởi chạy thành công!", "SUCCESS")
...
if __name__ == "__main__":
    print("=" * 65, flush=True)
    print("  TIẾN TRÌNH TRIỂN KHAI SCRIBD DOWNLOADER (192.168.1.250:9000)", flush=True)
    print("=" * 65 + "\n", flush=True)
